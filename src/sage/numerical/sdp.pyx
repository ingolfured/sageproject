...
.. csv-table::
    ...
    :meth:`~SemidefiniteProgram.write_lp`                  | Write the linear program as a LP file
    :meth:`~SemidefiniteProgram.write_mps`                 | Write the linear program as a MPS file
...
cdef class SemidefiniteProgram(SageObject):
    ...
    def new_variable(self, real=False, nonnegative=False, binary=False, integer=False, dim=1,name=""):
        ...
        v=SDPVariable(self, vtype, dim=dim,name=name)
        return v
    ...
    def set_binary(self, ee):
        r"""
        Sets a variable or a ``SDPVariable`` as binary.

        INPUT:

        - ``ee`` -- An instance of ``SDPVariable`` or one of
          its elements.
        ...
        if isinstance(e, SDPVariable):
            ...
            raise ValueError("e must be an instance of SDPVariable or one of its elements.")
    ...
    def is_binary(self, e):
        ...
        - ``e`` -- A variable (not a ``SDPVariable``, but one of its elements.)
    ...
    def set_integer(self, ee):
        r"""
        Sets a variable or a ``SDPVariable`` as integer.
    ...
    def set_nonnegative(self,ee):
        r"""
        Sets a variable or a ``SDPVariable`` as nonnegative.
    ...
    def set_min(self, v, min):
        ...
        - ``v`` -- a variable (not a ``SDPVariable``, but one of its
          elements).
    ...
    def get_min(self, v):
        ...
        - ``v`` -- a variable (not a ``SDPVariable``, but one of its elements).
    ...
    cpdef sum(self, L):
        ...
        - ``mip`` -- the :class:`SemidefiniteProgram` parent.
...
class SDPSolverException(RuntimeError):
    ...
    def __init__(self, value):
        ...
        EXAMPLE::
            ...
            sage: from sage.numerical.mip import SDPSolverException
...
cdef class SDPVariable(SageObject):
    ...
    def __cinit__(self, p, vtype, dim=1, name=""):
        ...
        - ``p`` -- the instance of ``SemidefiniteProgram`` to which the
